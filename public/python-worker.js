...
async function initialize() {
  ...
  for (const [name, code] of Object.entries(files)) {
    if (!/^toolstorm\/[a-z_]+\.py$/.test(name) || typeof code !== 'string')
      throw new Error('Invalid engine bundle');
  ...
  try {
    ...
    if (
      ...
      Object.keys(config).some(
        (key) =>
          !['scenario', 'seed', 'probability', 'max_calls'].includes(key),
      )
    ) {
    ...
    const result = py.runPython(
      'json.dumps(run_comparison(**json.loads(_config_json)))',
    );
    ...
    self.postMessage({
      id,
      error: String(error?.message || 'Python worker failed'),
    });
  }
};
